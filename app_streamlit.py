    ...
    try:
        ...
        with zipfile.ZipFile(temp_zip.name, 'w') as zipf:
            for idx, linha in df.iterrows():
                ...
                page = doc[0]
                page.insert_textbox(fitz.Rect(70, 75, 400, 100), nome)
                page.insert_textbox(fitz.Rect(70, 110, 400, 135), unidade)
                page.insert_textbox(fitz.Rect(155, 145, 400, 165), horario)
                page.insert_textbox(fitz.Rect(155, 170, 400, 195), plataforma)
                page.insert_textbox(fitz.Rect(230, 195, 500, 215), responsavel)
                page.insert_textbox(fitz.Rect(145, 220, 500, 245), f"({ddd}) {telefone}")

                if possui_plano:
                    page.insert_textbox(fitz.Rect(170, 245, 185, 255), "X")
                else:
                    page.insert_textbox(fitz.Rect(240, 245, 255, 255), "X")

                page.insert_textbox(fitz.Rect(110, 270, 400, 290), nome_plano)
                page.insert_textbox(fitz.Rect(170, 295, 400, 315), telefone_plano)

                if tipo_contato == "familiar":
                    page.insert_textbox(fitz.Rect(160, 320, 180, 330), "X")
                elif tipo_contato == "amigo":
                    page.insert_textbox(fitz.Rect(260, 320, 280, 330), "X")

                page.insert_textbox(fitz.Rect(85, 345, 400, 370), contato_nome)
                page.insert_textbox(fitz.Rect(100, 370, 400, 395), contato_telefone)
